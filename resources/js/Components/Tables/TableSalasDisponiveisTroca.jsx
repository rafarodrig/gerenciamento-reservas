import { Table, Button, Alert, Pagination, Card, Container } from 'react-bootstrap';
...
export default function TableSalasDisponiveisTroca({ data, onReservar, onPageChange }) {
  ...
  if (!salas || salas.length === 0) {
    return (
      <Container className='bg-white overflow-auto border rounded-4 shadow-sm p-3'  >
  ...
  return (
    <>
      <Container className='border bg-white rounded-4 shadow-sm p-3'>
        <Card className="border-0">
          <Card.Header className="d-flex bg-white align-items-center">
            <DoorOpen className="me-2 text-primary" />
            <strong>Salas disponíveis </strong>
          </Card.Header>
          <Card.Body className=" overflow-auto p-0 border-bottom" style={{ height: '350px' }}>
            <Table hover responsive className="table table-striped bg-white text-center align-middle mb-0">
              <thead>
                <tr>
                  <th>Sala</th>
                  <th>Tipo</th>
